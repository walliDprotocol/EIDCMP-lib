...
async function createCA({ admin_email, creatorWA, admin }) {
    try {
        const response = await makeAuthenticatedRequest('POST', '/ca', { admin_email, creatorWA, admin });
...
async function createTemplate({ cid, name , waAdmin, frontendProps }) {
    try {
        const response = await makeAuthenticatedRequest('POST', '/template', { cid, name, waAdmin, frontendProps });
        return response.data;
    } catch (error) {
