...
async function createCA({ admin_email, creatorWA, admin }) {
    try {
        const response = await makeAuthenticatedRequest('POST', '/create-ca', { admin_email, creatorWA, admin });
...
async function createTemplate({ cid, name , waAdmin, frontendProps }) {
    try {
        const response = await makeAuthenticatedRequest('POST', '/create-template', { cid, name, waAdmin, frontendProps });
        return response.data;
    } catch (error) {
